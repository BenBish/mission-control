...
export class SessionLogScanner {
  ...
  constructor(db: Database, options?: { sessionsGlob?: string; intervalMs?: number }) {
    this.db = db;
    this.sessionsGlob = options?.sessionsGlob || DEFAULT_SESSIONS_GLOB;
    this.intervalMs = options?.intervalMs || parseInt(process.env.SCAN_INTERVAL_MS || '') || DEFAULT_SCAN_INTERVAL_MS;
  }
  ...
  start(): void {
    ...
    console.log(`[Scanner] Starting session log scanner (interval: ${this.intervalMs}ms)`);
  ...
  async scan(): Promise<ScanResult> {
    if (this.scanning) {
      return { filesScanned: 0, newGenerations: 0, totalCost: 0, errors: ['Scan already in progress'] };
    }

    this.scanning = true;
    const result: ScanResult = { filesScanned: 0, newGenerations: 0, totalCost: 0, errors: [] };
   ...
   */
  private async scanFile(filePath: string): Promise<{ newGenerations: number; totalCost: number }> {
