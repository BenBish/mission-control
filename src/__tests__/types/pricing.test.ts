...
describe('Pricing', () => {
  ...
  describe('calculateCost', () => {
    test('should calculate cost for known model', () => {
      const cost = calculateCost('openrouter/anthropic/claude-haiku-4.5', 1000, 500);
    ...
    test('should calculate cost for multiple models', () => {
      const haikuCost = calculateCost('openrouter/anthropic/claude-haiku-4.5', 2000, 2000);
      const opusCost = calculateCost('openrouter/anthropic/claude-3-opus', 2000, 2000);
    ...
    test('should calculate cost with only input tokens', () => {
      const cost = calculateCost('openrouter/anthropic/claude-haiku-4.5', 1000, 0);
      expect(cost).toBeCloseTo(0.25, 3);
    });

    test('should calculate cost with only output tokens', () => {
      const cost = calculateCost('openrouter/anthropic/claude-haiku-4.5', 0, 1000);
      expect(cost).toBeCloseTo(1.25, 3);
    });

    test('should handle fractional tokens correctly', () => {
      const cost = calculateCost('openrouter/anthropic/claude-haiku-4.5', 500, 250);
  ...
  describe('Static Pricing Table', () => {
    ...
    test('Haiku should be cheaper than Sonnet', () => {
      ...
      expect(haiku.inputCostPer1kTokens).toBeLessThan(sonnet.inputCostPer1kTokens);
      expect(haiku.outputCostPer1kTokens).toBeLessThan(sonnet.outputCostPer1kTokens);
    });
    ...
    test('Sonnet should be cheaper than Opus', () => {
      ...
      expect(sonnet.inputCostPer1kTokens).toBeLessThan(opus.inputCostPer1kTokens);
      expect(sonnet.outputCostPer1kTokens).toBeLessThan(opus.outputCostPer1kTokens);
    });
  });
  ...
  describe('Model Pricing Comparisons', () => {
    test('should have correct relative pricing', () => {
      const models = [
        { name: 'claude-haiku-4.5', key: 'openrouter/anthropic/claude-haiku-4.5' },
        { name: 'claude-sonnet-4.5', key: 'openrouter/anthropic/claude-sonnet-4.5' },
    ...
    test('should calculate realistic costs', () => {
      ...
      const haiku1M = calculateCost('openrouter/anthropic/claude-haiku-4.5', 1_000_000, 0);
      expect(haiku1M).toBe(250); // $250 per 1M tokens

      // 1M output tokens on Haiku
      const haiku1MOut = calculateCost('openrouter/anthropic/claude-haiku-4.5', 0, 1_000_000);
