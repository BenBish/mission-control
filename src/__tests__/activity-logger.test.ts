...
describe("ActivityLogger", () => {
  ...
  describe("Token Tracking", () => {
    test("should log tokens and calculate cost", async () => {
      ...
      expect(activity?.cost?.usd).toBeLessThan(0.1); // Should be cheap (haiku pricing)
    });
    ...
    test("should calculate cost correctly for different models", async () => {
      ...
      expect(opus_activity?.cost?.usd).toBeGreaterThan(
        haiku_activity?.cost?.usd || 0,
      );
    });
  });
  ...
  describe("Session Summary", () => {
    ...
    test("should track actors in summary", async () => {
      ...
      expect(summary?.actors["agent:001"].actionsCount).toBeGreaterThanOrEqual(
        1,
      );
    });
