...
describe('API Routes', () => {
  ...
  describe('Activity Routes', () => {
    test('GET /api/activities should return activities', async () => {
      ...
      app._router.stack.forEach((layer: any) => {
        if (layer.route && layer.route.path === '/api/activities' && layer.route.methods.get) {
  ...
  describe('Cost Report Routes', () => {
    ...
    test('should aggregate costs by actor', async () => {
      ...
      const activities = await db.getActivities();
      const agent1Activities = activities.filter(a => a.actor.id === 'agent-1');
      const agent2Activities = activities.filter(a => a.actor.id === 'agent-2');
