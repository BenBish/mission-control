...
describe('Integration: Event-Based Pattern', () => {
  ...
  test('should log tool execution from events', async (done) => {
    ...
    eventEmitter.on('tool:end', (...args) => 
      eventLogger.onToolEnd(...args)
    );
    ...
    // Emit tool execution
    eventEmitter.emit('tool:start', 'read', { file: 'test.txt' }, sessionId, actor);
    await new Promise(r => setTimeout(r, 100));
    eventEmitter.emit('tool:end', { content: '...' }, null, { ...context, durationMs: 100 });
  });
