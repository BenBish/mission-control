...
| `GET /v1/organizations/usage_report/claude_code` | Admin | Productivity / estimated cost, not plan % | **Available (limited fit)** | Daily per-user sessions, LOC, tool accepts, **estimated_cost** by model. `customer_type`: `api` \| `subscription`. **No** “10% weekly used / resets in 4h” fields. **Requires an organization** — unavailable for individual/non-org accounts (see [BSH-348](#bsh-348-claude-code-collector--otel-vs-admin-analytics-api-vs-current-2026-09-10)) |
